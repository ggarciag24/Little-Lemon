...
import shoppingCart from "./images/shopping-cart-icon.png";
...
function OrderOnlinePage() {
    ...
    return(
        ...
        <div className="order-type">
            <button onClick={handleAdd}>Pick up</button>
            <button>Delivery</button>
        </div>
        <div className="menu-card-container">
                {foods.map((food) => {
                    return <OrderFoodCard img={food.img} name={food.name} price={food.price} description={food.description} />
                })}
            </div>
        </div>
    )
